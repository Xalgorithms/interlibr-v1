...
# container.
if [ "$1" = "up" ]; then
    docker-compose -f docker-compose-core.yml -f values-core-expose-kafka.yml up
elif [ "$1" = "down" ]; then
    docker-compose -f docker-compose-core.yml -f values-core-expose-kafka.yml down
else
